...
export class File extends Item {
	...
	constructor({ name = 'un-named file', textContent = '', source = null, size }) {
		super({name, size });
	...
	get copy() {
		return new File({ name:`${this.name} copy`, textContent: this.textContent, source: this.source, size: this.size });
	}
}
...
export class Directory extends Item {
	...
	constructor({ name = 'un-named directory', type = DIRECTORY_TYPE.DEFAULT, size = 0 }) {
		super({name, size})
	...
	get copy() {
		const dirCopy = new Directory({ name: `${this.name} copy`, type: this.type});
...
export class FileSystem {
	...
	createFile(fileName, options) {
		const newFile = new File({ name: fileName, ...options });
	...
	createDirectory(dirName, type = DIRECTORY_TYPE.DEFAULT) {
		const newDir = new Directory({ name: dirName, type });
